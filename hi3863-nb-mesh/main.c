...
static void* Soft_Mesh_Task(const char* arg)   //! 执行数据转发功能
{
    unused(arg);   // 忽略未使用的参数

    if (!is_soft_mesh_init)   // 进行首次初始化mesh网络
    {
        // 设置Mesh网络的SSID和密码，初始化Mesh网络
        if (mesh_init("AgricultureMesh", "12345678") != 0)
        {
    ...
    nb_detect_cnt++;
    if (nb_detect_cnt == NB_DETECT_CNT)   // 周期性检测NB模块是否可用，检测模块1损坏或者信号不好或者流量用尽
    {
        NB_Detect();
        nb_detect_cnt = 0;
    }
    ...
    {
        (void)uapi_watchdog_kick();          // 触发看门狗
        if (mesh_network_connected() == 0)   // 检查Mesh网络是否连接
        {
        ...
        {
            // 有NB模块，进行广播和上传收到的数据
            if (mesh_broadcast("Hello, Mesh!") != 0)   // 广播消息
            {
            ...
            char src_mac[7] = { 0 };   // 存储发送方MAC地址
            src_mac[6] = '\0';
            char data[200] = { 0 };   // 存储接收到的报文

            if (mesh_recv_data(src_mac, data) == 0)   // 接收数据
            {
                LOG("Received data from client: %s, MAC: %s\n", data, src_mac);   // 打印接收到的数据和发送方MAC地址
                // 根据数据长度区分报文类型并上传
                int len = strlen(data);
                if (len >= 120)
                {
                    NB_Upload_Soil_Data(data, len - 1);   // 如果数据长度大于等于120，认为是土壤数据
                }
                else if (len > 20)
                {
                    NB_Upload_Air_Data(data, len - 1);   // 如果数据长度大于20，认为是空气数据
                }
            }
            else
            {
        ...
        else
        {
            ...
            LOG("send payload to mac: %s\n", src_mac);                          // 打印发送目标MAC地址
            mesh_send_data(src_mac, pub_payload_air);                           // 发送空气数据
            printf("[%s] Air Data Payload: %s", __func__, pub_payload_air);     // 打印空气数据报文
            mesh_send_data(src_mac, pub_payload_soil);                          // 发送土壤数据
            printf("[%s] Soil Data Payload: %s", __func__, pub_payload_soil);   // 打印土壤数据报文
        }
...
static void Smart_Agriculture_Entry(void)   //! 设备初始化与线程创建
{
    ...
    // 创建数据转发线程
    taskid = osal_kthread_create((osal_kthread_handler)Soft_Mesh_Task, NULL, "Soft_Mesh_Task", 0xA000);
    ret = osal_kthread_set_priority(taskid, TASK_PRIO + 1);
    if (ret != OSAL_SUCCESS)
    {
        printf("[%s] Create Soft_Mesh_Task Failed.\n", __func__);
    }
